...
namespace BashSoft.IO.Commands
{
    ...
    {
        public PrintFilteredStudentsCommand(string input, string[] data, Tester judge, StudentsRepository repo, IOManager ioManager) : base(input, data, judge, repo, ioManager)
        {
        }
        ...
        private void TryParseParametersForFilterAndTake(string courseName, string filter, string takeCommand, string takeQuantity)
        {
            if (takeCommand.Equals("take"))
            {
                ...
                {
                    this.StudentsRepo.FilterAndTake(courseName, filter);
                }
                else
                {
                    int studentsToTake;
                    ...
                    if (hasParsed)
                    {
                        this.StudentsRepo.FilterAndTake(courseName, filter, studentsToTake);
                    }
                    else
                    {
